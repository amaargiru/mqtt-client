...
mqtt_keepalive = 5 * 60
mqtt_publish_topic = "ya_publisher/"
mqtt_topic_subscribe = "ya_publisher/#"  # Multi-level wildcard for cover all topic levels
mqtt_broker_reconnect_timeout = 10
publish_period = 10
...
def connect_mqtt():
    ...
    def on_message(client, userdata, message) -> None:
        logger.info(f"Message received \"{message.payload.decode()}\" from topic \"{message.topic}\"")
...
if __name__ == '__main__':
    ...
    while not connected_to_broker:
        try:
            ...
            logger.debug(f"Timeout {mqtt_broker_reconnect_timeout} seconds before reconnect")
            connected_to_broker = False
            time.sleep(mqtt_broker_reconnect_timeout)

    while True:
        time.sleep(publish_period)

        random_message = f"Random message_{random.randint(0, 100)}"
        result = client.publish(mqtt_publish_topic, random_message, qos=1)

        if result[0] == 0:
            logger.info(f"Message published \"{random_message}\" to topic \"{mqtt_publish_topic}\"")
        else:
            logger.error(f"Publish message error \"{random_message}\" to topic \"{mqtt_publish_topic}\"")
